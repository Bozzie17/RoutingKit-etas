...
int main(){
	...
	auto graph = simple_load_osm_car_routing_graph_from_pbf("maps/south-africa-latest.osm.pbf");
	...
	while(true){
        cout << "Please enter from_lat, from_long, to_lat, to_long:" << endl;
		...
		cout << "To get from "<< from << " to "<< to << " one needs " << distance/1000 << " seconds." << endl;
		cout << "The path is";
		for(auto x:path)
			cout << " " << x;
		cout << endl;
        cout << "     " << graph.latitude[path.back()] << ", " << graph.longitude[path.back()];
	}
}